...
routes.post("/users", requireAdminAuth, async (req, res) => {
  try {
    ...
    const roleUser = String(req.body?.role_user ?? "user").toLowerCase() === "admin" ? "admin" : "user";
...
routes.post("/auth/login", async (req, res) => {
  try {
    ...
    const rows = await sql`SELECT id_user, nome, email, senha, role_user, ativo FROM users WHERE email = ${email} LIMIT 1`;
    if (rows.length === 0 || rows[0].senha !== password) return res.status(401).json({ error: "Credenciais inválidas." });
    const user = rows[0];
...
routes.post("/auth/register", async (req, res) => {
  try {
    ...
    const created = await createUser({ nome, email, senha, role: "user", ativo: true });
